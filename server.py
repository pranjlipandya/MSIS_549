...
model = build_model()
model.load_weights('eco_model_new.h5')

class_indices = np.load('class_indices.npy', allow_pickle=True).item()

new_order_indices = sorted(class_indices, key=lambda x: class_indices[x])

class_info = {}
for old_idx, new_idx in class_indices.items():
    class_info[int(new_idx)] = original_class_info[int(old_idx)]
